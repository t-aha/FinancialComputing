...
class SteffInterp : public IInterp
{
public:
  SteffInterp() {}

  SteffInterp(const std::vector<double> &rArg, const std::vector<double> &rVal)
      : m_dL(rArg.front()), m_dR(rArg.back())
  {
    ...
    m_uS.reset(gsl_spline_alloc(gsl_interp_steffen, rArg.size()),
               &gsl_spline_free);
  ...
  {
    return new SteffInterp(rArg, rVal);
  }
