...
class CSpInterp : public IInterp
{
public:
  CSpInterp() {}

  CSpInterp(const std::vector<double> &rArg, const std::vector<double> &rVal)
      : m_dL(rArg.front()), m_dR(rArg.back())
  {
    ...
    m_uS.reset(gsl_spline_alloc(gsl_interp_cspline, rArg.size()),
               &gsl_spline_free);
  ...
  {
    return new CSpInterp(rArg, rVal);
  }
