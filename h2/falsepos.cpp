...
double fvalue (double dX, void *param)
{
...
cfl::Root
prb::falsepos (double dFuncErr, unsigned iMaxSteps)
{
  return cfl::Root (

      new AbsRoot (dFuncErr, iMaxSteps, gsl_root_fsolver_falsepos));
}
...
cfl::Root
prb::falsepos (double dAbsErr, double dRelErr, unsigned iMaxSteps)
{
  return cfl::Root (

      new RelRoot (dAbsErr, dRelErr, iMaxSteps, gsl_root_fsolver_falsepos));
}