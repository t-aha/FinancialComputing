...
double brvalue (double dX, void *param)
{
...
cfl::Root
prb::brent (double dFuncErr, unsigned iMaxSteps)
{
  return cfl::Root (
      new AbsRoot (dFuncErr, iMaxSteps, gsl_root_fsolver_brent));
}

cfl::Root
prb::brent (double dAbsErr, double dRelErr, unsigned iMaxSteps)
{
  return cfl::Root (

      new RelRoot (dAbsErr, dRelErr, iMaxSteps, gsl_root_fsolver_brent));
}